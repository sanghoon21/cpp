...
// +Ch 07 - original: 원래 몬스터월드.
//          + 실습문제: 에너지 아사 기능 추가 (에너지 0 되면 아사)
// +Ch 08 - original: 동적 할당, Matrix 클래스 추가.
//          + 실습문제: 에너지 아사 기능 추가. Ch 07과는 다른 방식. (몬스터가 아사 시 동적으로 해제됨.)
// +Ch 09 - original: 여러 몬스터 추가.(좀비, 뱀파이어, 처녀귀신, 가로 세로 강시)
//          + 실습문제: 여러 몬스터 추가.(스몸비, 샹시, 나만의몬스터 - 늙은처녀귀신)
// +Ch 10 - original: 실행시간 다형성(vitual 키워드) 추가, 조작 가능한 플레이어 추가 (Human 클래스)
//          + 실습문제: Tuman 클래스, moveHuman() 으로 2명의 플레이어 가능.
// +Ch 11 - original: 좌표상의 이동 거리에 따라 휴식.
//          + 실습문제: X
// +Ch 12 - original: 랭킹 기능 추가(플레이어 2명의 랭킹 모두 저장 가능). 예외처리 사용한 랭킹 파일 관리 기능 추가. - 랭킹 파일(MonsterWorld.rnk) 존재하지 않으면 예외처리.(관리자 비밀번호 입력받아서 파일 생성?)
//          + 실습문제: 원래 랭킹 파일에 랭킹 저장 실패 시 예외처리.(관리자 비밀번호 입력받아서 또 다른 새 랭킹파일 만들어서 랭킹 저장. - 새 파일명: ~~~.rnk   ex) newrankfile.rnk )
//                    원래 랭킹 파일의 쓰기 권한 없애서 저장 안되도록(예외 발생하도록) 함.
...
//   => 비밀번호를 쳐서 맞으면 새 파일 이름 입력하고 새 파일에 랭킹 저장. 틀리면 그냥 저장 없이 종료.
