...
//*        몬스터와 맵의 상호작용, 출력, 아사 체크를 처리하는 클래스.
class MonsterWorld 
{
...
// 4-29. isDone() - 남은 아이템이 0이면 true 반환 - 끝.
...
// 4-32. print() - Canvas로 맵과 몬스터 그리기, 이동 횟수, 아이템 수, 몬스터 수 출력.
    void print() 
    {
        canvas.clear();
        for (int y = 0; y < yMax; y++)
...
//! (5) 일반 몬스터는 move로 이동하고, 플레이어는 kbhit()로 입력을 확인한 후 moveHuman을 호출하여 이동.
//!  - 일반 몬스터는 for (int k = 0; k < nMon - 2; k++)를 통해 move 호출.
//!   - 입력이 ESC(27)로 시작하면 화살표 키로 간주, getchar()로 '['를 건너뛰고 다음 문자를 읽어,
//!      오른쪽 플레이어(pMon[nMon-1])의 moveHuman 호출.
//!   - ESC가 아니면 왼쪽 플레이어(pMon[nMon-2])의 moveHuman 호출.
//!  (5)번은 아님, 이동 후 checkStarvation으로 에너지 0인 몬스터 제거, print로 상태 출력.
//!  isDone()(아이템 소진) 또는 nMon < 2(플레이어 1명 이하) 시 게임 종료.
// 4-36. play()
//  - 게임 루프: 몬스터 이동, 플레이어 입력 처리, 아사 체크, 상태 출력, 종료 조건 체크.
    void play(int maxwalk, int wait) 
    {
        ...
        {
            if (nMon >= 2) // 플레이어가 최소 2명 남아 있어야 함
            { 
