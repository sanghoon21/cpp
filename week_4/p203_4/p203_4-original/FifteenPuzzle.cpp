#include "Ranking.h"
#include <unistd.h>  // usleep() 사용
#include <stdlib.h>
#include <stdio.h>
#include <chrono>     // 시간 측정을 위한 chrono 라이브러리
#include <iostream>   // 입력 및 출력 처리
#include <thread>     // 시간 지연을 위한 thread 라이브러리

#define DIM 4  // 4x4 퍼즐

enum Direction { Left = 'a', Right = 'd', Up = 'w', Down = 's' };
static int map[DIM][DIM];
static int x, y;    
static int nMove;
static std::chrono::time_point<std::chrono::high_resolution_clock> tStart, tLastMove;  // 시작 및 마지막 이동 시간

static void init() {
    for (int i = 0; i < DIM * DIM - 1; i++)
        map[i / DIM][i % DIM] = i + 1;
    map[DIM - 1][DIM - 1] = 0;
    x = DIM - 1; y = DIM - 1;
    srand(time(NULL));
    tStart = std::chrono::high_resolution_clock::now();  // 게임 시작 시간
    tLastMove = tStart;  // 첫 번째 이동 시간을 시작 시간과 동일하게 설정
    nMove = 0;
}

static void display() {
    system("clear");
    printf("\tFifteen Puzzle\n\t");
    printf("--------------\n\t");
    for (int r = 0; r < DIM; r++) {
        for (int c = 0; c < DIM; c++) {
            if (map[r][c] > 0)
                printf("%3d", map[r][c]);
            else printf("   ");
        }
        printf("\n\t");
    }
    printf("--------------\n\t");

    // 게임이 시작되었을 때만 시간 계산
    if (nMove > 0) {
        auto tNow = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed = tNow - tStart;
        printf("\n\t이동 횟수:%6d\n\t소요 시간:%6.1f초\n\n", nMove, elapsed.count());
    } else {
        printf("\n\t이동 횟수:%6d\n\t소요 시간: 0.0초\n\n", nMove);
    }
}

static bool move(char dir) {
    if (dir == 'd' && x > 0) {
        map[y][x] = map[y][x - 1];
        map[y][--x] = 0;
    } 
    else if (dir == 'a' && x < DIM - 1) {
        map[y][x] = map[y][x + 1];
        map[y][++x] = 0;
    } 
    else if (dir == 'w' && y < DIM - 1) {
        map[y][x] = map[y + 1][x];
        map[++y][x] = 0;
    } 
    else if (dir == 's' && y > 0) {
        map[y][x] = map[y - 1][x];
        map[--y][x] = 0;
    } 
    else return false;

    nMove++;  // 이동 횟수 증가

    // 게임이 시작되지 않았다면 게임 시작 시간을 설정
    if (nMove == 1) {
        tStart = std::chrono::high_resolution_clock::now();
    }

    // 이동 후 마지막 이동 시간을 갱신
    tLastMove = std::chrono::high_resolution_clock::now();
    return true;
}

static void shuffle(int nShuffle) {
    for (int i = 0; i < nShuffle; i++) {
        char keys[] = {'w', 'a', 's', 'd'};
        char key = keys[rand() % 4];
        if (move(key) == false) { i--; continue; }
        display();
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
    }
}

static bool isDone() {
    for (int r = 0; r < DIM; r++) {
        for (int c = 0; c < DIM; c++) {
            if (map[r][c] != r * DIM + c + 1)
                return (r == DIM - 1) && (c == DIM - 1);
        }
    }
    return true;
}

static char getDirKey() { return getchar(); }

int playFifteenPuzzle() {
    init();
    display();
    printRanking();
    printf("\n 엔터 키를 눌러 섞기를 시작하세요...");
    getchar();
    shuffle(100);  // 퍼즐 섞기
    printf("\n 엔터 키를 눌러 게임을 시작하세요...(시작 후 한 번 움직이면 시간이 지납니다)");
    getchar();

    // 게임 시작 시점
    tStart = std::chrono::high_resolution_clock::now();
    tLastMove = tStart;  // 첫 이동 시점과 동일
    nMove = 0;

    while (!isDone()) {
        move(getDirKey());
        display();
    }

    // 게임 종료 후 총 소요 시간 계산
    auto tEnd = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = tEnd - tStart;
    return addRanking(nMove, elapsed.count());
}
